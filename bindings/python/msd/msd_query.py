...
def msd_query(baseURL: str, sql: str, h: Handler[R] = None) -> Generator[R, None, None] :
  """
  Query data from msd.
  ...
  for t in parse_reader(response.raw) :
    if h is not None :
      yield h(t)
    else :
      yield t
...
async def async_msd_query(baseURL: str, sql: str, h: Handler[R] = None) -> Generator[R, None, None] :
  """
  ...
  async with aiohttp.ClientSession() as session:
    async with session.post(endpoint, json={"query": sql}, headers={
      ...
      if response.status != 200:
        raise Exception(f"Query failed: {response.text}")
      async for t in parse_reader_async(response.content) :
        if h is not None :
          yield h(t)
        else :
          yield t