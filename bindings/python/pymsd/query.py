...
from typing import AsyncGenerator, Callable, Generator, Tuple, overload, TypeVar
from .reader import parse_reader, parse_reader_async
from .const import MSD_QUERY_PATH, MSD_USER_AGENT
from .dataframe_adaptor import MsdTable
import logging
...
) -> Generator[Tuple[str, str, R | MsdTable], None, None]:
  """
  Query data from msd.
...
def async_query(
  baseURL: str, sql: str
) -> AsyncGenerator[Tuple[str, str, MsdTable], None]: ...


@overload
def async_query(
  baseURL: str, sql: str, h: Callable[[MsdTable], R]
) -> AsyncGenerator[Tuple[str, str, R], None]: ...


async def async_query(
  baseURL: str, sql: str, h: Callable[[MsdTable], R] | None = None
) -> AsyncGenerator[Tuple[str, str, R | MsdTable], None]:  # type: ignore
  """
