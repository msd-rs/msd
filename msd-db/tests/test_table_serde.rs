...
fn build_table() -> Table {
  table!(
    ...
    {name:"price", kind:d64, data:vec!["1.23", "2.00", "3.00"]},
...
fn test_to_bytes() -> anyhow::Result<()> {
  ...
  let deserialized_table_json: Table = serde_json::from_str(&json_str)?;
  println!(
    "Deserialized table from JSON: {:?}",
    deserialized_table_json
  );

  let deserialized_table: Table = DbBinary::from_bytes(&bytes)?;
